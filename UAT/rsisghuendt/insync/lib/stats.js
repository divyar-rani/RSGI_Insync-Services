...
class stats extends base {
    ...
    async __find_files(base, policy) {
        let ret = {};
        let ppath = path.join(base, policy.endorsement_id+'-0.json');
        if (fs.existsSync(ppath)) ret[policy.endorsement_id+'-0.json'] = 1;

        ppath = path.join(base, policy.policy_id+'-0.xml');
        if (fs.existsSync(ppath)) ret[policy.policy_id+'-0.xml'] = 1;

        ppath = path.join(base, policy.endorsement_id+'-req.txt');
        if (fs.existsSync(ppath)) ret[policy.endorsement_id+'-req.txt'] = 1;

        ppath = path.join(base, policy.endorsement_id+'-res.txt');
        if (fs.existsSync(ppath)) ret[policy.endorsement_id+'-res.txt'] = 1;
