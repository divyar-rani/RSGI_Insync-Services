...
class fgenPolicy extends twigbase {
    ...
    async _process_service(service, policy) {
		...
		console.log("********************** After __add_additional_data", policy.policy_id)
    ...
    async __get_fg_status(service, jx, policyId, subid, attr) {
		...
		else errorMessage = await utils.jpath_value(jx, "soapenv:Envelope.soapenv:Body.ns2:FGUWResponseVO.errorDetailVOList", service.target.strobjs)|'' ;
		console.log("******************* ",status," ** " , errorMessage);
        let policy_no = "";	
        if (errorMessage){
			if(status == 'SUCCESS' || errorMessage.includes('Posted Successfully') || errorMessage.includes('Policy already Exists in Master')) {
