...
class is3 extends istore {
   ...
   /* async connect() {
        if (!this.sparams.accessKeyId) return;
        this.s3 = new aws.S3(this.sparams);
        await this.createBucket();
    }*/
    // Alternative approach using AWS SDK's built-in credential chain

async connect() {

    if (!this.sparams.iamRole) return;

    console.log('Setting up with role:', this.sparams.iamRole);

    const credentials = new aws.ChainableTemporaryCredentials({

        params: {

            RoleArn: this.sparams.iamRole,
    ...
    console.log('S3 client created with chained credentials provider');
    ...
    async __check_if_bucket_exists(name) {
        try {
            ...
            if (ret.Status != 'Enabled') await this.__enable_bucker_versioning(name);
            this.bucket_created = true;
            return true;
        } catch (e) {}
        return false;
    }
    async _test_credentials() {    try {        console.log('Testing S3 credentials with listBuckets operation');        const data = await this.s3.listBuckets().promise();        console.log('ListBuckets succeeded, found', data.Buckets.length, 'buckets');        return true;    } catch (e) {        console.log('ListBuckets failed:', e.code, e.message);        return false;    }}    


    async createBucket() {
        if(!this.s3 || this.bucket_created) return null;
   	const credentialsValid = await this._test_credentials();    if (!credentialsValid) {        console.log('Skipping bucket creation due to invalid credentials');        return;    } 
       let params = {
