...
class twigbase {
        ...
        if (!this.tmpl[policy.product_id][service.uuid] || service.disable_cache) {
            ...
            for (let i=0; i<service.twigs.length; i++) {
                ...
                if (!fs.existsSync(tfile)) {
                    this.ish.__log('error', 'twig file does not exist '+tfile, policy.endorsement_id, this.constructor.name);
                    return null;
                }
    ...
    async __transform_all(service, policy, jdata) {
        ...
        let attrs = await this.ish.get_attrs(policy.endorsement_id) || {};
        ...
        for (let i=0; i<this.tmpl[policy.product_id][service.uuid].length; i++) {
            ...
            this.__fs_log(service, policy.endorsement_id+'-'+i+'.json', JSON.stringify(data), policy);
            this.__fs_log(service, policy.endorsement_id+'-'+i+'.xml', ret||'', policy);
    ...
    __fs_log(service, fname, data, policy) {
        ...
        let mdt = policy?.endorsement_id ? policy.endorsement_id.substring(policy.endorsement_id.length-2) : 'AA';
    ...
    async __transform(service, policy, twigtxt, rendered) {
        ...
        try {
            ...
            this.__fs_log(service, policy.endorsement_id+'.json', JSON.stringify(policy), policy);
            this.__fs_log(service, policy.endorsement_id+'.twig', twigtxt, policy);
            this.__fs_log(service, policy.endorsement_id+'.trace', ret.stack_trace, policy);

            let parts = ret.stack_trace.split("soapenv:Envelope");
            if (parts.length >= 2) {
                this.ish.__log('error', parts[0]+parts[parts.length-1], policy.endorsement_id, this.constructor.name);
            }
            
            this.ish.__log('error', ''+ret.stack_trace, policy.endorsement_id, this.constructor.name);
            this.ish.__log('error', service.name + ':' + ret.error, policy.endorsement_id, this.constructor.name);
            await this.ish.__state(policy.endorsement_id, 'transform-failed', undefined, 'l3');
            istatsd.event(['transform.twig.error']);
            return null;

        } catch (e) {
            this.ish.__log('exception', e, policy.endorsement_id, this.constructor.name);
            await this.ish.__state(policy.endorsement_id, 'transform-failed', undefined, 'l3');
    ...
    async __check_ignored(policy) {
        ...
        if (this.ish.trace) console.log(this.ish.name+':', policy.endorsement_id, policy.policy_no, prodname);
        ...
        if (reason) {
            this.ish.__log('info', reason, policy.endorsement_id, this.constructor.name);
            this.ish.__state(policy.endorsement_id, reason);
            await this.ish.mark(policy.endorsement_id);
            return false;
        }
        return true;
    }
    ...
    async __put_on_hold(service, policy, reason) {
        this.ish.__log('info', reason, policy.endorsement_id, this.constructor.name);
        this.ish.__state(policy.endorsement_id, 'hold');
        return true;
    }
    ...
    async process_next() {
        ...
        for (let pid of pids) {
            ...
            //
            if (!await this.ish.__lock(policy.endorsement_id, this.constructor.name)) {
                ...
                //
                this.ish.__log('warning', 'locked', policy.endorsement_id, this.constructor.name);
                continue;
            }
            ...
            try {
                ...
                if (await this.__process_policy(policy) !== null) {
                    ...
                    await this.ish.reschedule(policy.endorsement_id, 10*60, 'rescheduled', this.constructor.name);
                }
                this.stats.process += (performance.now()-time);
            } catch (e) {
                if (typeof e === 'string' && e.startsWith('hold')) {
                    this.ish.__log('hold', e, policy.endorsement_id, this.constructor.name);
                    this.ish.__state(policy.endorsement_id, e);
                    await this.ish.mark(pid);
                    ...
                    await this.ish.reschedule(policy.endorsement_id, rwait, 'rescheduled ' + msg + ' ' + rwait + 's', this.constructor.name);

                    this.ish.__state(policy.endorsement_id, this.constructor.name+'-failed', undefined, 'l3');
                } else if (typeof e === 'string' && e.startsWith('state ')) {
                    await this.ish.mark(pid);
                    this.ish.__state(policy.endorsement_id, e.substring(6).trim(), undefined, 'l3');
                } else {
                    await this.ish.mark(pid);
                    istatsd.event(['process.exception']);
                    this.ish.__log('exception', e, policy.endorsement_id, this.constructor.name);
                    this.ish.__state(policy.endorsement_id, 'generic', undefined, 'l3');
                }
            }
            await this.ish.__unlock(policy.endorsement_id, this.constructor.name);
            ...
            if (pids.length > 0) {
                console.log('('+this.threadId+')'+this.constructor.name.padStart(10), 
                    ...
                    policy.endorsement_id.padEnd(15)
                    );
            }
        }
    ...
    async __save_attributes(service, jx, policyId, subid) {
        ...
        for (let attr of service.target.attributes) {
            ...
            if (val && typeof val === 'string') {
                let parts = val.split('$$');
                val = parts[0];
                if (parts.length > 1 && parts[1].indexOf("SUCCESSFULLY")) {
    ...
    async __failed(service, policy, type, message) {
        ...
        await this.ish.__log(type, message, policy.endorsement_id, service.name);
        await this.ish.__state(policy.endorsement_id, service.name+'-failed', undefined, 'retry');
    ...
    async __invoke_url(service, xdata, policy, subid) {
        let policyId = policy.endorsement_id;
        ...
        try {
            ...
            await this.ish.__log('exception', e.message||e, policy.endorsement_id, service.name);
            
            // check if this is a retriable error
            //
            await this.__check_error_retry(service, e.message||e, policy.endorsement_id);
    ...
    async __check_service_status(service, policy, subid, xdata) {
        ...
        // Added added this in new usiness same thillai added for endorsement on 27-08-2024
	if (service.name == 'proposal-fetch') return false;

        let notfound = service.target.attributes.filter(x => x.mandatory && !attrs[x.name+key]);
        if (notfound.length == 0) this.ish.__log('info', this.ish.name+': already completed', policy.endorsement_id, this.constructor.name);
    ...
    async __copy_to_sftp(service, policy, xdata, subid) {
        ...
        await this.__save_attributes(service, xdata, policy.endorsement_id, subid);
    ...
    async __copy_to_folder(service, policy, xdata, subid) {
        ...
        if (service.target?.folder.use_suffix) {
            bpath = path.join(bpath, policy.endorsement_id.substring(policy.endorsement_id.length-2));
        }
    ...
    async __call_service(service, policy, xdata, subid) {
        ...
        await this.ish.__state(policy.endorsement_id, service.name + '-failed', undefined, this.errbkt);
        this.stats.api += (performance.now()-start);
    ...
    async __match_conditional(service, policy, xdata, attrs) {
        ...
        await this.ish.__log('info', 'skipped-condition-'+service.name, policy.endorsement_id, service.name);
        return false;
    }
    ...
    async __process_policy(policy) {
        ...
        if (skipped) {
            this.ish.__log('warning', this.ish.name+': skip (matching service not found)', policy.endorsement_id, this.constructor.name);
            this.ish.__state(policy.endorsement_id, 'skipped');
            return true;
        }
        
        if (final) {
            await this.ish.__log('info', 'completed', policy.endorsement_id, this.name);
            await this.ish.__state(policy.endorsement_id, 'completed', attr);
        }
