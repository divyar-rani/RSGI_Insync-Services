...
class insync {
    ...
    async __update_custom_fields(def, policy, cur) {
        ...
        if (keys.length > 0) { // found one or more values to be updated into is_policy
            ...
            console.log('cust fields: ', policy.policy_id, values);
        } else {
            console.log('no cust fields: ', policy.policy_id);
        }
    ...
    async _download_policy(def, policyId) {
        ...
        if (!ret || ret.status != 0) {
            ...
            if (policy.proposal_no.startsWith('@') && !policy?.issue_date ) {
                console.log("********************* Test Policy",policyId);
                await this._add_to_purgatory(def, policyId, 'Test Policy', 'cutoff');
                return;
            }
            ...
            //
            if (!policy.policy_no && !def.include_paid) {
                await this._add_to_purgatory(def, policyId, 'policy not issued yet', 'issue_date');
                return;
            }
            ...
            if (p) {
                ...
                let params = [policyId, policy.policy_no, prodname, policy.product_id, policy.issue_date, policy.proposal_no, def.name];
			...
			policy.issue_date = policy.issue_date;
            ...
            try {
                ...
                if (!sdata) throw('failed to store in s3 ' + policy.policy_id);
    ...
    async __update_payment_details(def, policy) {
        ...
        for (let pd of policy.payment.details) {
            if (!await db.exec(sql, [policy.policy_id, pd.payment_details_id, pd.payment_id, prodname, pd.c_ts||null, pd.c_ts])) {
    ...
    async __next_batch(def, l_ts, depth) {
       ...
       // if (def.include_paid) url += '&policy.payment_id='+encodeURIComponent('!')+'&quote.status=^0,2,8';
        ...
        if (diff >= 8) {
            ...
            console.log('    using time range:', l_ts, e_ts );
        ...
        //let ret = await utils.iget(url, def);
	    url = def.server + '/api/v1/policy/list2?';	
		let reqData = {
		     'quote.status': 2,
              'order': 'u_ts desc',
              'schema': '1',
              'fields': 'issue_date,u_ts,policy_id',
              'u_ts': '('+l_ts
		    }
        //let ret = await utils.iget(url, def);
    ...
    async _update_paid_polciies(def, start, end) {
        ...
        let ret = await utils.iget(def.server+'/b2c/api/v1/stats/query/policy_paid?start='+encodeURIComponent(start)+'&end='+encodeURIComponent(end), this.cust[def.name]);
        if (ret?.status !== 0) {
    ...
    async _update_issued_polciies(def, start, end) {
		...
		let ret = await utils.iget(def.server+'/api/v1/stats/query/policy_issued?start='+encodeURIComponent(start)+'&end='+encodeURIComponent(end), this.cust[def.name]);
        ...
        let pids = ret.data.map(x => x.policy_id);
